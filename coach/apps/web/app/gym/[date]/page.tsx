...
export default async function GymDatePage({ params }: { params: { date: string } }) {
  ...
  const { session, lastPerf, hasSaved, catalog } = await getGymDayData(date);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <Link href="/gym/exercises" className="btn-ghost ml-auto text-xs">Exercises</Link>
      </div>
      <GymLogger initial={session} lastPerf={lastPerf} catalog={catalog} />
    </div>
  );
}
